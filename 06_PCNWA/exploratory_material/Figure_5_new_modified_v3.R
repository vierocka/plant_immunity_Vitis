...
# * = shared predicted co-receptor of both SOBIR1 and LRIP1 (RLP-a, GSO2, RLP-b, RLP-c; 4 of 7 SOBIR1 partners)
...
n_sobir <- sum(pc_df$super_node == "SOBIR1/LRIP1 recognition node")
...
# Edge linetype: solid = contact-filtered PAE < 10 Å; dashed = PAE ≥ 10 Å (SOBIR1–LRIP1)
...
af2_edges_df <- data.frame(
  from = c(
    ...
    # SARD4 – EXLRR11
    "SARD4",
    # LRIP1 – predicted co-receptors (11)
    rep("LRIP1", 11),
    # SOBIR1 – predicted co-receptors (7)
    rep("SOBIR1", 7),
    # SOBIR1 – LRIP1 cross-module (PAE > 10 Å)
    "SOBIR1"
  ),
  to = c(
