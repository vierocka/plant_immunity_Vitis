...
#          + right-side annotations: functional group and WGCNA module size
...
gene_df <- data.frame(
  id = c(
    ...
    # LRIP1 complex (AF2-predicted partners)
  ...
  module_size = c(
    ...
    34, NA, 57, 55, NA, NA, 32, NA, NA   # LRIP1 complex
  ),
...
# * = also predicted AF2 partner of LRIP1 (shared SOBIR1+LRIP1 co-receptor)
# MM4 pattern: UUU/EEE/UUU — specific to Rpv12 and Rpv12+1+3, not Rpv12+1

# ── 1. Read expression data ──────────────────────────────────────────────────

fch_file <- "STARmap1/geneID_log2FCH_FDR_Rpv12_Rpv12Rpv1_Rpv1213_0624_againstRef.csv"
...
# Right annotation: functional group bar + module size
...
rlog_file <- "data_files/Rlogs.csv"
...
string_raw <- read_tsv(
  "06_PCNWA/string-db/results/string_interactions_short.tsv",
...
af2_status <- c(
  ...
  "Receptor kinase","AF2 co-receptor","AF2 co-receptor",               # LRIP1, NP181-a, NP181-b
...
# Super-node divider: separate EDS1 (12 genes, rows 18-29) from SOBIR1/LRIP1 (17 genes, rows 1-17)
n_eds1  <- sum(pc_df$super_node == "EDS1/SAR signaling node")    # 12
n_sobir <- sum(pc_df$super_node == "SOBIR1/LRIP1 recognition node") # 17

super_colors <- c(
  "EDS1/SAR signaling node"     = "#FFF3E0",
  "SOBIR1/LRIP1 recognition node"= "#E8F4FD"
)

Pc <- ggplot(pc_df) +
  # Background stripe per super-node
  geom_rect(
    aes(xmin = 0, xmax = 11.8,
  ...
  annotate("text", x = col_x_af2, y = header_y, label = "Evidence",
           hjust = 0.5, size = 3, fontface = "bold") +
  annotate("text", x = col_x_mod, y = header_y, label = "Module size",
           hjust = 0.5, size = 2.8, fontface = "bold") +
  # Header separator
  geom_hline(yintercept = n_genes + 0.7, color = "grey30", linewidth = 0.5) +
  coord_cartesian(xlim = c(-0.5, 12.1), ylim = c(0.4, header_y + 0.3), clip = "off") +
  theme_void(base_size = 9) +
  theme(
    plot.background  = element_rect(fill = "white", color = NA),
    panel.background = element_rect(fill = "white", color = NA),
...
outdir <- "06_PCNWA/exploratory_material"
