...
mi <- read.csv("06_PCNWA/combat_protected/results/GCNA_rebuild_9459panel_FDR_datadriven_rcutoff/modules_info_protected_ComBat_FDR.csv")
sig_genes <- mi$geneID[mi$fdr < 0.05]
message(length(sig_genes), " significant modules (FDR<0.05).")

all_members <- readRDS("06_PCNWA/combat_protected/results/GCNA_rebuild_9459panel_FDR_datadriven_rcutoff/module_members_protected_ComBat_FDR.rds")
stopifnot(setequal(names(all_members), sig_genes))
module_members <- all_members
...
write.csv(hub_table, "06_PCNWA/hub_genes_top1pct_6600modules_r0.8077_FDR.csv", row.names = FALSE)
