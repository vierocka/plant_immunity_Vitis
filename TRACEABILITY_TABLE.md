...
- **Verified** — script, input, and output all confirmed present and
  consistent this session (path checked, and/or script re-run).
- **Present, not re-run** — script/input/output located and read, but not
  actually re-executed this session to reproduce the numbers.
- **Descriptive** — no statistical test; a schematic, summary table, or
  illustrative figure.
- **Not independently verified** — not checked this session; flagged so
...
|---|---|---|---|
| ST1 | Main table + `Chitarrini_2020`/`Shi_2024`/`Froussios_2019` per-dataset sample/accession overview | Sample metadata; matches `Froussios_2019/README.md`, `Chitarrini2020/README.md`, `RUN1_RPV1_microvine_2024/` accession citations built this session | Verified (accessions cross-checked against ENA/SRA this session for all three datasets) |
| ST2 | AED: inter-genotype, within-genotype, gene overlaps, noise-injection test, 3 external-dataset validations, gene concentration | `03_AED/analysis/` (combat_protected/comparison), `03_AED/Chitarrini_AED/`, `03_AED/RUN1_RPV1_microvine_2024_AED/`, `03_AED/Froussios_2019_AED/` | Verified (this session: ran both Chitarrini_AED scripts end-to-end after fixing a real broken-path bug; confirmed real output) |
...
| ST5 | Full per-gene DESeq2 differential-transcription results (9,459 DEGs), per the proof's own in-text citation | not present locally | **Accepted as a known gap, not pursued** (user decision) |
| ST6 | DESeq2 primary DE results + group/category summary | `02_Normalization_and_DGEA/DESeq2_classic/results/` (`*_deseq2_NB_BE_model.csv` files) | Verified (this session: pulled real up/down DEG counts directly from these files for jupyter_nb fix — e.g. Rpv12 0h 2765up/1359down) |
...
| ST8 | AF2-Multimer PPI screen: positive hits, domain architecture, within/cross-module/negative/positive-control/stickiness-panel results, technical failures | `PPIs/` (this session), raw screen run on RAMSES (not locally reproducible — flagged, not re-run) | Present, not re-run — copied wholesale as the canonical results table; methodology cross-checked against manuscript-drafted text this session |
| ST9 | rlog values, DE genes (2 contrasts), overlapping genes, Jaccard + significance | `PPIs/Chitarrini/`, `02_Normalization_and_DGEA/Chitarrini_DE/` | Verified (this session: re-ran the Jaccard/TPM-Spearman scripts producing this table's underlying numbers) |
...
| ST11 | Froussios2019 size factors + log2 SF-norm expression | `Froussios_2019/ST11_size_factors_and_expr_export.R` | Present, not re-run (script confirmed present/kept this session during the Froussios_2019 folder cleanup) |

## Supplementary figures checked this session

| Item | Description | Script | Status |
|---|---|---|---|
| SF1 | Read-mapping QC by batch | `01_QC_and_Filtering/Batch_effects/read_checks.R`; reproduced independently in `jupyter_nb/` Section 1 | Verified (jupyter_nb path/column check this session) |
| SF2 | PCA across normalization methods | `02_Normalization_and_DGEA/DESeq2_classic/scripts/PCA_protected_ComBat.R`; reproduced in `jupyter_nb/` Section 2 | Verified (jupyter_nb path check this session) |
| SF3 | AED permutation nulls (A), noise-injection negative control (B), transcriptional noise vs. dosage (C) | `03_AED/analysis/combat_protected/scripts/SF3_rebuild.R` | Present, not re-run this session (header/paths touched in an earlier session) |
| SF6 | Network robustness panels (module-size distributions, STRING precision) | `06_PCNWA/string-db/scripts/SF6_network_robustness_rebuild.R`; underlying checks in `06_PCNWA/network_robustness/` | Verified (this session: fixed conversation-snapshot comments; underlying checks 1-5 in `network_robustness/README.md` independently re-verified as PASSED with real numbers in an earlier session) |
...
- **Splicing-junction finding** (07_exploratory_Splicing_junctions/):
  confirmed this session to be **absent from the current manuscript
  revision** — an earlier draft (different journal target) included a
  splicing-junction finding (SRG1) that was dropped. Correctly excluded
  from the main text; kept in the repo labeled exploratory.
- **De novo assembly / unmapped-reads characterization**
  (de_novo_assembly/): confirmed this session to be mentioned in the
  manuscript text only as a brief exploratory QC paragraph (not a
  figure/table), matching what's in the repo.
